    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-500 text-sm">Input data pemasok lengkap dengan termin pembayaran.</p>
            </div>
        </div>
        ...
        <form action="{{ route('suppliers.store') }}" method="POST">
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $autoCode) }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" readonly>
                        <p class="text-xs text-slate-400 mt-1">Otomatis digenerate sistem.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Nama PT / Toko <span class="text-red-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: PT. Sinar Jaya Abadi" required>
                    </div>

                    {{-- Kontak Person --}}
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Sales Contact Person</label>
                        <input type="text" name="contact_person" value="{{ old('contact_person') }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Nama Salesman">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">No. Telepon / HP</label>
                        <input type="text" name="phone" value="{{ old('phone') }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">
                    </div>

                    {{-- Email & Alamat --}}
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Email</label>
                        <input type="email" name="email" value="{{ old('email') }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-slate-700 mb-2">Alamat Lengkap</label>
                        <textarea name="address" rows="3" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('address') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mb-6">
                ...
                <div class="p-6">
                    <div class="max-w-md">
                        ...
                        <div class="relative">
                            <input type="number" name="term_days" value="{{ old('term_days', 0) }}" min="0" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 pr-12 font-bold text-slate-700">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-3 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 shadow-md transition-colors">Simpan Supplier</button>
            </div>
