<x-app-layout>
    <div x-data="{ 
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <button @click="openCreate = true" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-lg text-sm font-semibold shadow-sm flex items-center gap-2 transition-colors">
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-xs text-slate-500 truncate max-w-xs">{{ $supplier->address }}</td>
                            <td class="px-6 py-4 text-center">
                                <button @click="openEdit = true; selectItem({{ $supplier }})" class="text-slate-400 hover:text-indigo-600 transition-colors">
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-slate-400">Belum ada data supplier.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="openCreate" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <form action="{{ route('suppliers.store') }}" method="POST" class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                    @csrf
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                        <h3 class="text-lg font-bold text-slate-900 mb-4">Tambah Supplier Baru</h3>
                        <div class="space-y-4">
                            <div class="grid grid-cols-3 gap-3">
                                <div class="col-span-1">
                                    <label class="block text-xs font-bold text-slate-700 uppercase">Kode</label>
                                    <input type="text" name="code" required class="w-full rounded-lg border-slate-300 text-sm" placeholder="SUP-001">
                                </div>
                                <div class="col-span-2">
                                    <label class="block text-xs font-bold text-slate-700 uppercase">Nama PT / Toko</label>
                                    <input type="text" name="name" required class="w-full rounded-lg border-slate-300 text-sm" placeholder="PT. Sinar Jaya">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase">Sales Contact Person</label>
                                <input type="text" name="contact_person" class="w-full rounded-lg border-slate-300 text-sm" placeholder="Nama Salesman">
                            </div>
        ...
        <div x-show="openEdit" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                    ...
                    <form id="editForm" x-bind:action="'/suppliers/' + (selectedItem ? selectedItem.id : '')" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                            <h3 class="text-lg font-bold text-slate-900 mb-2">Edit Supplier</h3>
                            <div class="space-y-4 mt-4">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-slate-700 uppercase">Sales Contact Person</label>
                                    <input type="text" name="contact_person" x-bind:value="selectedItem ? selectedItem.contact_person : ''" class="w-full rounded-lg border-slate-300 text-sm">
                                </div>
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs font-bold text-slate-700 uppercase">No. Telepon</label>
                    ...
                    <div class="bg-slate-50 px-4 py-3 sm:px-6 flex flex-col-reverse sm:flex-row sm:justify-between items-center gap-3">
                        <form x-bind:action="'/suppliers/' + (selectedItem ? selectedItem.id : '')" method="POST" onsubmit="return confirm('Hapus supplier ini?');">
                            @csrf @method('DELETE')
                        ...
                        <div class="flex gap-2 justify-end w-full sm:w-auto">
                            ...
                            <button type="submit" form="editForm" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Simpan</button>
                        </div>
                    </div>
                </div>
            </div>
